...
data class Orientation(val rotA: Pair<Axis, Int>, val rotB: Pair<Axis, Int>, val offset: List<Int>) {
  fun manhattanTo(other: Orientation): Int {
    val psum = (0..2).map {
      (offset[it] - other.offset[it]).absoluteValue
    }

    val sum = psum.sum()

//    doLog { "Manhattan: $offset to ${other.offset} == $psum == $sum" }
...
val max = reoriented.values.flatMap { a ->
  reoriented.values.mapNotNull { b ->
    if (a != b) {
      a.manhattanTo(b)
    } else null
  }
