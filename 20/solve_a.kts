...
val final = (0..1).fold(pixels) { pixels, iteration ->
  val iter = enhance(pixels, iteration)
  printPhoto(iter)
  iter
}

// Slice off the border :eyeroll:
//val (xRange, yRange) = ranges(final)
//val xlose = 2 * (xRange.last - xRange.first - 4)
//val ylose = 2 * (yRange.last - yRange.first - 6)
